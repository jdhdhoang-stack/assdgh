...
const synthesizeGemini = async (options: SynthesizeOptions): Promise<string> => {
    const apiKey = options.token || process.env.GEMINI_API_KEY;
    if (!apiKey) {
        throw new TTSError("Thiếu Gemini API Key.");
    }

    const ai = new GoogleGenAI({ apiKey });
    const voiceName = options.speaker.replace('gemini-', '');
    ...
    try {
        ...
        if (base64Audio) {
            const bytes = b64decode(base64Audio);
            const blob = new Blob([bytes], { type: 'audio/mpeg' });
            return URL.createObjectURL(blob);
        } else {
            throw new TTSError("Không nhận được dữ liệu âm thanh từ Gemini.");
        }
    } catch (error: any) {
        console.error("Gemini TTS Error:", error);
        throw new TTSError(`Gemini TTS Error: ${error.message || 'Unknown error'}`);
    }
};

export const synthesizeChunk = (options: SynthesizeOptions): Promise<string> => {
    if (options.speaker.startsWith('gemini-')) {
        return synthesizeGemini(options);
    }
